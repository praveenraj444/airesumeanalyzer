...
def generate_auto_rewrite_suggestions(analysis):
    """AI automatically analyzes resume and provides rewrite suggestions"""
    if not GEMINI_AVAILABLE:
        return {'error': 'AI service not available', 'items': []}
    
    if not analysis:
        return {'error': 'No resume analysis found', 'items': []}
    
    job_role = analysis.get('job_role', 'Professional')
    ...
    for category_skills in skills.values():
        skills_list.extend(category_skills)
    skills_text = ', '.join(skills_list[:15]) if skills_list else 'various skills'
    
    prompt = f"""You are an expert resume writer and career coach.

Analyze this candidate's resume profile for a {job_role} position and provide 
...
- Experience: {experience} years
- Skills: {skills_text}
- Action Verbs Found: {action_verbs_count}

Provide 5 specific resume improvement suggestions. For each suggestion, give:
1. A common weak bullet point example
2. An improved version with strong action verbs and quantifiable results
3. Why the improved version is better

Format your response EXACTLY like this for each suggestion:

WEAK: [weak bullet point example]
IMPROVED: [strong improved version with numbers]
REASON: [why this is better]
---

Provide exactly 5 suggestions separated by ---
"""
    
    try:
        response = GEMINI_MODEL.generate_content(prompt)
        return parse_rewrite_response(response.text)
    except Exception as e:
...
def parse_rewrite_response(text):
    ...
    for section in sections:
        ...
        item = {'weak': '', 'improved': '', 'reason': ''}
        
        lines = section.strip().split('\n')
        for line in lines:
            line = line.strip()
            if line.startswith('WEAK:'):
                item['weak'] = line.replace('WEAK:', '').strip()
            elif line.startswith('IMPROVED:'):
                item['improved'] = line.replace('IMPROVED:', '').strip()
            elif line.startswith('REASON:'):
                item['reason'] = line.replace('REASON:', '').strip()
        
        if item['weak'] and item['improved']:
            items.append(item)
